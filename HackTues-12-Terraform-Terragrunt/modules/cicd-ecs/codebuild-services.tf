...
resource "aws_codebuild_project" "codebuild_service" {
  ...
  source {
    ...
    buildspec = templatefile("${path.module}/buildspec-service.yml", {
      AWS_DEFAULT_REGION                          = var.region
      AWS_ACCOUNT_ID                              = var.account_id
      IMAGE_REPO_NAME                             = "${var.environment}-${var.app_name}-${each.key}-repo"
      CONTAINER_NAME                              = "${var.environment}-${var.app_name}-${each.key}"
      TASK_DEFINITION_ARN_FAMILY_WITHOUT_REVISION = var.service_task_arn_without_revision[each.key]
  ...
  logs_config {
    cloudwatch_logs {
      group_name = aws_cloudwatch_log_group.codebuild_service_logging.name
      status     = "ENABLED"
    }
  }
}
